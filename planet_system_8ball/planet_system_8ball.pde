...
void showLegend() {
  ...
  text("CONTROL DE CAMARA", width/2, height/2 - 160);
  text("MANEJAR INTESIDAD DE BRILLO", width/2, height/2);
  text("ENTER - Start/Pause", width/2, height/2 + 160);

  textFont(createFont("Arial", 24));
  text("ZOOM - W(in) A(out)", width/2, height/2 - 120);
  text("ROTAR - S(left) D(right)", width/2, height/2 - 80);
  text("INCREMENTAR - P", width/2, height/2 + 40);
  text("DISMINUIR - M", width/2, height/2 + 80);
}
